...
| 功能 | 说明 |
|---|---|
| 🪟 桌面壳 | Electron 包装 DSH Web GUI，DSH 未启动时显示离线提示页，托盘常驻 |
| 🧩 插件系统 | 从 **GitHub 仓库**（`owner/repo` / `owner/repo@tag`）、**zip 直链**、**本地目录**安装插件 |
| 🛡️ 安全协议 | **每次安装前**自动检查兼容性（OS/架构/Electron/DSH/Node 版本）→ **安装后自动运行插件测试** → 测试通过才启用；**任何一步失败自动回滚删除**，零残留 |
| ⭐ 推荐中心 | 内置推荐插件与 Skill（翻译/代码审查/总结/日报/搜索引导…），一键安装或开关 |
| 🧠 上下文记忆 | 只读监听 DSH 对话；估算用量达到阈值（默认 70%）时**自动生成简洁交接摘要**，写入 `memory/HANDOFF.md` 留给下一个会话；可选 DeepSeek API Key 做 LLM 总结、可选自动注入对话框 |
| 🎨 动态壁纸 | 视频/网页壁纸挂载到桌面图标之下（类似 Wallpaper Engine），支持多显示器虚拟屏幕，由内置 wallpaper-plugin 开关控制 |
